...
import { Affixes, CorpusApiWord, PartOfSpeech, WordData } from "@/types";
...
fs.readFileSync("./src/quran-data/morphology.txt")
  ...
  .forEach((line, lineIndex, lineArray) => {
    ...
    const grammaticalCase = getGrammaticalCase(line) as "NOM" | "GEN" | "ACC";
...
// write data
fs.writeFile("./src/quran-data/data.json", JSON.stringify(data), function (err) {
  if (err) throw err;
  console.log("./src/quran-data/data.json");
});

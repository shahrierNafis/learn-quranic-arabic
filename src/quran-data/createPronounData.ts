...
import { CorpusApiWord } from "@/types";
import fs from "fs";
const pronounData: {
  [key: string]: {
    [key: string]: {
...
// write data
fs.writeFile("./src/quran-data/pronounData.json", JSON.stringify(pronounData), function (err) {
  if (err) throw err;
  console.log("./src/quran-data/pronounData.json");
});
